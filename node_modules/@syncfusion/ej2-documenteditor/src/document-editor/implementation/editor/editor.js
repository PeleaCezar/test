...
var Editor = /** @class */ (function () {
    ...
    Editor.prototype.insertTextInternal = function (text, isReplace, revisionType) {
        ...
        if (this.documentHelper.protectionType === 'FormFieldsOnly' && this.selection.isInlineFormFillMode()) {
            ...
            var resultText = this.getFormFieldText(inline);
    ...
    Editor.prototype.moveContent = function (lineWidget, startOffset, endOffset, insertIndex, paragraph) {
        ...
        for (var i = 0; i < lineWidget.children.length; i++) {
            ...
            if (endOffset <= count + endIndex - startIndex) {
                ...
                for (var i_2 = 0; i_2 < lineWidget.children.length; i_2++) {
                    textBox.line.children[i_2] = lineWidget.children[i_2];
                }
                break;
            }
    ...
    Editor.prototype.onApplyParagraphFormat = function (property, value, update, isSelectionChanged) {
        ...
        var previousPara = currentPara.previousWidget;
        var listId;
        ...
        if (property === 'leftIndent') {
            if (selection.paragraphFormat.listId !== -1 && update) {
                if (!isNullOrUndefined(previousPara) && listId === -1 || isNullOrUndefined(previousPara)) {
                    var list = this.documentHelper.getListById(selection.paragraphFormat.listId);
    ...
    Editor.prototype.applyParaFormatProperty = function (paragraph, property, value, update) {
        ...
        if (property === 'afterSpacing') {
            ...
            var currentPara = format.ownerBase;
            var previousPara = currentPara.previousWidget;
            ...
            if (isNullOrUndefined(previousPara) || (!isNullOrUndefined(previousPara)
                ...
                while (!isNullOrUndefined(currentPara) && currentPara.paragraphFormat.listFormat.listId != -1) {
                    if (!isListChanged) {
                        this.documentHelper.layout.clearListElementBox(currentPara);
                        this.onListFormatChange(currentPara, value, format);
                    }
                    previousPara = currentPara;
                    currentPara = currentPara.nextWidget;
                    if (!isNullOrUndefined(currentPara)) {
                        if (previousPara.paragraphFormat.listFormat.listLevelNumber != currentPara.paragraphFormat.listFormat.listLevelNumber) {
                            var currentList = this.documentHelper.getListById(currentPara.paragraphFormat.listFormat.listId);
                            var listLevel = this.documentHelper.layout.getListLevel(currentList, currentPara.paragraphFormat.listFormat.listLevelNumber);
                            this.copyFromListLevelParagraphFormat(currentPara.paragraphFormat, listLevel.paragraphFormat);
                            isListChanged = true;
                        }
                    }
                    this.layoutItemBlock(previousPara, false);
                }
            }
            else {
                ...
                if (canRemoveParaMark) {
                    ...
                    for (var i = 0; i < nextParagraph.childWidgets.length; i++) {
                        ...
                        if (nextParagraph.characterFormat.revisions.length > 0) {
                            for (var i_3 = 0; i_3 < nextParagraph.characterFormat.revisions.length; i_3++) {
                                for (var j = 0; j < nextParagraph.characterFormat.revisions[i_3].range.length; j++) {
                                    nextParagraph.characterFormat.revisions[i_3].range[j].ownerBase = paragraph;
                                }
                                paragraph.characterFormat.revisions.push(nextParagraph.characterFormat.revisions[i_3]);
                            }
                        }
    ...
    Editor.prototype.updateFormFieldResult = function (field, value) {
        ...
        if (textElement instanceof TextElementBox) {
            if (isNullOrUndefined(textElement.nextNode)) {
                var line = textElement.line.paragraph.nextWidget.childWidgets[0];
